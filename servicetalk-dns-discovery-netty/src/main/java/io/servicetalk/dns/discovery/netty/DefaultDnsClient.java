...
final class DefaultDnsClient implements DnsClient {
    ...
    private final Map<String, List<InetAddressWatcher>> srvInetWatchersMap = new HashMap<>(8);
    ...
    public Publisher<ServiceDiscovererEvent<InetAddress>> dnsQuery(final String address) {
        final InetAddressWatcher entry;
        if (nettyIoExecutor.isCurrentThreadEventLoop()) {
            if (closed) {
                return failed(new IllegalStateException(DefaultDnsClient.class.getSimpleName() +
                        " closed!"));
            }
            entry = new InetAddressWatcher(address, __ -> { });
        } else {
            entry = new InetAddressWatcher(address, __ -> { });
    ...
    public Publisher<ServiceDiscovererEvent<InetSocketAddress>> dnsSrvQuery(final String serviceName) {
        final SrvWatcher srvWatcher;
        if (nettyIoExecutor.isCurrentThreadEventLoop()) {
            if (closed) {
                return failed(new IllegalStateException(DefaultDnsClient.class.getSimpleName() +
                        " closed!"));
            }
            srvWatcher = new SrvWatcher(serviceName);
        } else {
            srvWatcher = new SrvWatcher(serviceName);
            nettyIoExecutor.asExecutor().execute(() -> {
                if (closed) {
                    srvWatcher.close0(false);
                }
            });
        }
        return srvWatcher.publisher.flatMapMerge(srvEvent -> {
            assertInEventloop();

            if (srvEvent.isAvailable()) {
                if (closed) {
                    return Publisher.failed(
                            new IllegalStateException(DefaultDnsClient.class.getSimpleName() + " closed!"));
                } else {
                    InetAddressWatcher entry = new InetAddressWatcher(srvEvent.address().hostName(),
                            this::removeSrvEntry0);
                    addSrvEntry0(entry);
                    return entry.publisher.map(inetEvent -> new DefaultServiceDiscovererEvent<>(
                            new InetSocketAddress(inetEvent.address(), srvEvent.address().port()),
                            inetEvent.isAvailable()));
                }
            } else {
                final List<InetAddressWatcher> inetWatchers = srvInetWatchersMap.remove(srvEvent.address().hostName());
                if (inetWatchers != null) {
                    for (InetAddressWatcher inetWatcher : inetWatchers) {
                        inetWatcher.close0(true);
                    }
                }
    ...
    private void addSrvEntry0(final InetAddressWatcher entry) {
        assertInEventloop();

        srvInetWatchersMap.computeIfAbsent(entry.inetHost, k -> new ArrayList<>(2)).add(entry);
    }

    private void removeSrvEntry0(final InetAddressWatcher entry) {
        assertInEventloop();

        final List<InetAddressWatcher> inetWatchers = srvInetWatchersMap.get(entry.inetHost);
        if (inetWatchers == null) {
            return;
        }
        inetWatchers.remove(entry);
        if (inetWatchers.isEmpty()) {
            srvInetWatchersMap.remove(entry.inetHost);
        }
    }
    ...
    private final class SrvWatcher {
        private final String serviceName;
        private final SrvWatcherEntriesPublisher entriesPublisher;
        private final Publisher<ServiceDiscovererEvent<HostAndPort>> publisher;

        SrvWatcher(final String serviceName) {
            this.serviceName = serviceName;
            entriesPublisher = new SrvWatcherEntriesPublisher();
            publisher = entriesPublisher.flatMapConcatIterable(identity());
        }

        void close0(boolean tryGenerateInactiveEvents) {
            entriesPublisher.close0(tryGenerateInactiveEvents);
        }

        private final class SrvWatcherEntriesPublisher extends EntriesPublisher<HostAndPort> {
            @Override
                                ...
                                try {
                                    ...
                                    for (DnsRecord dnsRecord : dnsRecords) {
                                        if (!SRV.equals(dnsRecord.type()) || !(dnsRecord instanceof DnsRawRecord)) {
                                            // todo(scott): should we just continue if type != SRV? for now be strict.
    ...
    private final class InetAddressWatcher {
        private final String inetHost;
        private final Consumer<InetAddressWatcher> removalConsumer;
        private final InetAddressWatcherEntriesPublisher entriesPublisher;
        private final Publisher<ServiceDiscovererEvent<InetAddress>> publisher;

        InetAddressWatcher(final String inetHost, final Consumer<InetAddressWatcher> removalConsumer) {
            this.inetHost = inetHost;
            this.removalConsumer = removalConsumer;
            entriesPublisher = new InetAddressWatcherEntriesPublisher();
            publisher = entriesPublisher.flatMapConcatIterable(identity());
        }
        ...
        private final class InetAddressWatcherEntriesPublisher extends EntriesPublisher<InetAddress> {
            @Override
            protected AbstractEntriesPublisherSubscription newSubscription(
                ...
                return new AbstractEntriesPublisherSubscription(subscriber) {
                    ...
                    @Override
                    protected void removeFromWatchers() {
                        removalConsumer.accept(InetAddressWatcher.this);
                    }
                };
            }
        }
    ...
    private abstract class EntriesPublisher<T>
        ...
        final void close0(boolean tryGenerateInactiveEvents) {
            assertInEventloop();
            ...
            if (oldSubscriber != null) {
                ...
                if (tryGenerateInactiveEvents) {
                    ...
                    safeOnError(oldSubscriber, new ClosedServiceDiscovererException(DefaultDnsClient.this +
                                    " has been closed!"));
                }
            }
        }
