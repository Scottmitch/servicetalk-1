...
import static io.servicetalk.concurrent.internal.EmptySubscription.EMPTY_SUBSCRIPTION_NO_THROW;
    ...
    void switchTo(Subscription next) {
        ...
        for (;;) {
            ...
            if (currSourceRequested == CANCELLED) {
                ...
                final long currRequested = requested;
                if (currRequested < 0) { // invalid requestN is pending, deliver to each subscription.
